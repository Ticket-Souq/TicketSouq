...
public interface PayoutRepository extends JpaRepository<Payout, UUID> {
    ...
    @Query("SELECT DISTINCT p.organization FROM Payout p WHERE p.organization IS NOT NULL")
    java.util.List<String> findDistinctOrganizations();

    @Query("SELECT DISTINCT p.organization FROM Payout p WHERE p.organization IS NOT NULL AND LOWER(p.organization) LIKE LOWER(CONCAT('%', :search, '%'))")
    ...
    @Query("SELECT DISTINCT p.eventId FROM Payout p WHERE p.organization = :org AND p.eventId IS NOT NULL")
    java.util.List<UUID> findDistinctEventIdsByOrganization(@org.springframework.data.repository.query.Param("org") String org);

    @Query("SELECT COALESCE(SUM(p.netAmount), 0) FROM Payout p WHERE p.organization = :org AND p.status = 'COMPLETED'")
    java.math.BigDecimal sumPaidByOrganization(@org.springframework.data.repository.query.Param("org") String org);

    @Query("SELECT COALESCE(SUM(p.netAmount), 0) FROM Payout p WHERE p.eventId = :eventId AND p.status = 'COMPLETED'")
    ...
    java.util.List<Payout> findByStatus(String status);

    java.util.List<Payout> findByOrganizationAndStatus(String organization, String status);
}
